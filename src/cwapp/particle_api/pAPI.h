...
namespace PAPI {
    ...
    public:
        ...
        ///
        /// The Particle API uses a pseudo-random number generator. The returned number is a function of the numbers already returned. If you start two threads, each with a ParticleContext_t they will both generate the same particles if given the same commands. If this is not desired, call Seed() on both of them with different seed values.
        /// The API currently uses the C standard library random number generator, whose state is per-thread, so all contexts in the thread share the same random number seed.
        ...
        ///
        /// The action_list_num must have already been generated using GenActionLists. Most calls other than actions and state setting calls cannot be made between a call to NewActionList() and the corresponding call to EndActionList().
    ...
    public:
        ...
        ///
        /// Copy particles from the specified particle group, p_src_group_num, to the current particle group. Only copy_count particles, starting with number index are copied. Of course, the number of particles actually copied is bounded by the available space in the current particle group, and the number of particles actually in the source particle group. The particles are added, in sequential order, to the end of the current group. index is the index of the first particle in the source particle group to be copied.
        ...
        ///
        /// Generates p_group_count new particle groups and returns the particle group number of the first one. The groups are numbered sequentially, beginning with the number returned. Each particle group is set to have at most max_particles particles. Call SetMaxParticles() to change this.
        ...
        ///
        /// GetParticles() returns the number of particles copied to application memory. Of course, the number of particles actually returned is bounded by count and by the number of particles actually in the particle group minus index.
        /// If verts, color, vel, size or age is NULL then the respective field will not be returned. index and count must be at least 0 and less than the number of particles. index + count must be less than the number of particles.
        ...
        //IK -- return just Particle_t array
        size_t GetParticlePointer(Particle_t *&ptr);
    ...
    public:
        ...
        /// Also, actions such as RandomDisplace() that modify a particle's position directly, rather than modifying its velocity vector, may yield unsatisfying results when used with Bounce().
        ///
        ...
        /// Since this action changes particle positions, rather than changing their velocities and depending on the Move() action to change the positions, unsatisfying results may occur when used with the Avoid() or Bounce() actions.
        ...
        /// This allows particles to die when they turn around, get too fast or too slow, etc. For example, use a sphere domain centered at the origin with a radius equal to the minimum velocity to kill particles that are too slow.
        ...
        ///
        /// Many rendering systems require rendering transparent particles in back-to-front order. The ordering is defined by the eye point and the look vector. These are the same vectors you pass into gluLookAt(), for example.
        /// The vector from the eye point to each particle's position is computed, then projected onto the look vector. Particles are sorted back-to-front by the result of this dot product.
        ...
        /// When the Source action is called within an action list, the particle attribute domains used are those that were current when the Source command was called within the NewActionList() / EndActionList() block instead of when CallActionList() was called. Note that this is unlike OpenGL.
